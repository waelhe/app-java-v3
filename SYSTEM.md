...
- **Redis بدورين (بعد V31):** جلسات + 13 مخبأة مسماة (`application.yml`). الإبطال **بعد الالتزام حصراً**: `CacheInvalidationRelay` (`@TransactionalEventListener(AFTER_COMMIT)`) ينشر `CacheInvalidationRequested` — لا `@CacheEvict` مباشر على الخدمات (أزيلت 30 إزالة في PR #179). (ادُّعاء قديم هنا بأن Redis كان «خزين Quartz JDBC» كان غير دقيق — المتجر كان دائماً في PostgreSQL من V21؛ صُحّح مع إزالة المتجر نفسه.)
