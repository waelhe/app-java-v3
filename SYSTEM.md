...
- **الإنتاج حي (آخر تحقق 2026-09-05، جولة #210):** الإنتاج يقلع بكود `8ada038` (#209 — دفعه المستخدم إلى fork النشر): التحقق الحي عبر النقاط العمومية — `/actuator/modulith` = **401** (النقطة موجودة ومؤمّنة = كود #209 منشور) + liveness/readiness 200 + jwks 200 (معرّف النشرة نفسه غير قابل للقراءة هذه الجولة: توكن GraphQL المحلي 403 — بوابة التدوير الموثقة)؛ فحص health الكامل 503 = **البند 3 الموثق** (MAIL_* placeholders — بوابة المزوّد بيد المستخدم، ليست حالة جديدة). **main الآن `de49bd0` يحمل #210 (دورة حياة الأحداث — صفر ترحيلات)** — يُنشَر بدفعه المستخدم المعتاد إلى fork النشر (القناة ذاتها)؛ توكن Railway متوفر في scripts/secrets للتحقق الكامل فور النشر إن عاد التوكن صالحًا، وإلا فالفحص الثلاثي العمومي + `/actuator/modulith` هما سلطة التحقق.
