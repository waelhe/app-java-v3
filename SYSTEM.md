...
- **المرحلة 4 ✅ (PR المرحلة-4 — مفتوح):** D6 منفّذ: حارس fail-fast مقيد بـ prod في `jwkSource` (نمط المُهيّئ الرسمي نفسه) + فحص CI `JwkSourceProdHardeningTest` (تخفيف سجل مخاطر §8) + runbook `keys/README.md` + تصحيح دين ARCHITECTURE الموثقي (`RotatingJWKSource` الوهمية في 4 مواضع → ADR-004 Revised يسجل التصميم المنفّذ بصدق).
