...
- **الإنتاج حي:** `https://app-java-v3-production-d020.up.railway.app` — نشر نشط `c82d9831` (SUCCESS) من main `27b5a155` عبر fork النشر المتزامن. بروفايل **prod** مفعّل. liveness/readiness 200 UP، jwks بمفتاح RSA دائم `kid=marketplace-jwt` (أُعيد التحقق 2026-09-04؛ فحص health الكامل = DOWN بعنصر البريد الموثق أدناه — البند 3). **فجوة موثقة (تحققت 2026-09-04):** main تقدّم إلى `19f7cea` بثلاث دمجات بعد النشر (#197 التوثيق + #198 تحديث الإصدارات + #199 تصميم Dockerfile الرسمي) بينما fork النشر (`waelhe88-coder/app-java-v3`) ما زال على `27b5a155` — **إعادة النشر الحاملة للتحديثات بيد المستخدم** (توكن الجلسة بلا صلاحية دفع على fork النشر؛ ترقية خدمتي PostgreSQL 17→18 و Redis 7→8 على البوابة بيد المستخدم كذلك — انظر §1).
- **آلية البناء الحاكمة (التصميم على main `19f7cea` = #199):** `railway.toml` → `builder=DOCKERFILE` + Dockerfile **رباعي المراحل**: build (temurin 25-jdk + Railway cache mount لـ `~/.m2` بالصيغة الرسمية `id=s/<service-id>-<path>`) / extractor (طبقات `jarmode=tools` الرسمية) / **trainer (تدريب AOT cache لـ Java 25+ بوصفة Boot 4.1 الرسمية: `exit=onRefresh` + وصفات lifecycle لكل اعتمادية + lazy-initialization لتفادي الأنظمة البعيدة عند البناء)** / runtime (temurin 25-jre، مستخدم غير جذري `app`). ENTRYPOINT خالص `"java", "-jar", "app.jar"` — الوصفة الرسمية الحرفية. (النشر الحي `c82d9831` بُني بالتصميم الثلاثي السابق — الرباعي يدخل الإنتاج عند أول إعادة نشر من main بعد `19f7cea`.) البناء الفعلي مثبت في `serviceManifest` لكل نشور مصدر.
...
- **حوكمة النشر:** أي إعادة نشر تمر عبر fork (`waelhe88-coder/app-java-v3` — دفعه يتطلب بيانات حسابه، خارج توكن الجلسة الحالية)؛ CI سلطة الحكم على كل رأس قبل الدمج؛ فحص ما بعد النشر: liveness + jwks + OIDC discovery (read-only عبر curl).
