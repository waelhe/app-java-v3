...
- **تأسيس كامل على المسار الرسمي 🏗️ (PR #185 — قيد التطوير):** المُهيّئ نفسه يتولى الآن **التأسيس** (لا التدوير فقط): غائب ⇒ `withId(a7bd8b0d-…)` + التعريف الكامل (grants/redirect/scopes/name/auth-methods — نقل SQL→Java بند مستقل في PR) + باني الإعدادات؛ converge-on-boot للهوية فقط؛ حارس save موسّع (سرّ ∨ خرائط). `R__seed_oauth2_client.sql` أُخلي من العميل (admin باقٍ) — يغلق الدين الأمني (سرّ مضمن) والفجوة الحية (id-token). مرجع: `docs/security/oauth2-client-bootstrap-spec.md` §4.1/§4.2/§4.4 (سجل الانحرافات).
