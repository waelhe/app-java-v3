...
- **الإنتاج حي (آخر تحقق 2026-09-04، دفعة truth-sync):** `https://app-java-v3-production-d020.up.railway.app` — نشر نشط **`80171be7` (SUCCESS) من main `f9c5d34` (#203)** عبر fork النشر المتزامن حرفياً (`check_deploy_fork` = EXACTLY synced — تحقق حي). بروفايل **prod** مفعّل، إقلاع 10.634s، PostgreSQL 18.6 + Redis 8.2 على الشبكة الداخلية (طبقة البيانات أدناه). liveness/readiness 200 UP، jwks بمفتاح RSA دائم `kid=marketplace-jwt`، OIDC discovery 200 (تحقق ثلاثي 2026-09-04؛ فحص health الكامل = DOWN بعنصر البريد الموثق — البند 3 من الديون). **سلسلة النشور الحاملة لـ main:** `c82d9831` (27b5a155) ← فشل `bf2b0acd` (41eeb05 — أُصلح جذره في #201) ← `6c814e28` (349052b) ← `442c3665` (قطع طبقة البيانات) ← `51b5496d` (707e052 = #202 IaC) ← **`80171be7` (f9c5d34 = #203 V28) — النشط** (دليل إقلاعه: «Migrating schema… to version 28 — modulith event archive» + «Successfully applied 1 migration» + صفر أخطاء أرشفة).
