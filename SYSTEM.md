...
- **المرحلة 2 ✅** (PR #184 → مدمج `773558e` 2026-09-02، CI أخضر ×2): F-B مغلقة بالمعنى المقصود — S1–S6 تمرّن على **صف المُهيّئ الفعلي** (`@TestPropertySource` + `ApplicationRunner` يأسّس أثناء الإقلاع، لا عميل مُصنّع في كود الاختبار): قراءة الخريطة الكاملة (8 مفاتيح)، PKCE مفروض على الصف الفعلي، consent موجب/سالب بمستخدمين مختلفين، القيم الظرفية، convergence بصف قديم مرضوض. المواصفة: `docs/security/oauth2-client-bootstrap-spec.md`.
- **المفتوح (بقرار D9/D6):** المرحلة 3 (عميل مستهلك بموجب D9=(أ) confidential/BFF، يغلق F-C — والتوصية الرسمية المحررة حرفياً في دليل SAS «How-to: SPA with PKCE»: «We recommend the backend for frontend (BFF) pattern») أو المرحلة 4 (D6: keystore دائم + فحص CI يحظر `JWT_KEYSTORE_*` الفارغة في الإنتاج — سجل مخاطر الخطة §8). سلوك PKCE **ساكن حتى يولد مستهلك** في المرحلة 3.
