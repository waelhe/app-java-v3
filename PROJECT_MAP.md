...
**صفر كسر:** صفر وحدات نطاق، صفر واجهات، صفر مخطط، صفر اعتماديات جديدة — طبقة رصد وإعداد حصراً؛ البوابات المحلية: infra 52/52 + app 51/51 + المؤشر 10/10 (الانحرافان البيئيان الموثقان للسياق الكامل — CI سلطة الحكم). القواعد خاملة حتى توصيل مثيّر Alertmanager (دين موثق بنقطة إغلاق: بوابة اختيار المستهلك، تقترن بالبوابة C).

**خارطة الطريق بعد هذه الطبقة:** الديون المعلنة كلها بنقاط إغلاق (slo.md §6): توصيل Alertmanager + لوحات Grafana خلف بوابة اختيار المستهلك (البوابة C — لا تُبنى لوحات لأداة غير مختارة)، **runbooks تفاعلية للحوادث (المرحلة ج من خارطة الإغلاق — الاسترداد والحوادث)**، معايرة كل العتبات بعد أول canary (خطة الإطلاق).
