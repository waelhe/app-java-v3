...
**الحرّاس (11):** اختباران بأسلوب S2 في `AuthorizationServerLoginGateIntegrationTest` (V8 انضمت لـschema-locations البوابة): (أ) سك توكنات → تعطيل إداري → **طلب التحديث التالي 400 invalid_grant + محاولة الدخول التالية /login?error** → إعادة تفعيل → **دخول جديد يُسك توكنات و/me = 200** (معيارا القبول 1+3)، (ب) 409 عند تعطيل آخر ADMIN نشط مع ثبات قدرته على الدخول (معيار 2). + 7 وحدة في `UserServiceTest` (شكل القلب عبر المدير، شكل صف التدقيق بالسبب، الحالات السلبية الأربع، القيد العدّي باتجاهيه) + `AdminControllerTest` (تفويض SPI بالفاعل) + `@MockitoBean UserDetailsManager` لتوصيل شريحة الوحدة + دبوس `ObservationCoverageFilesTest` (`user.status.update` — قاعدة التحديث المقصود في نفس الدفعة).
