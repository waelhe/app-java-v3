...
public class NotificationService {
    ...
    public void onBookingCreated(BookingCreatedEvent event) {
        createIfRecipientPresent(event.consumerId(), "Booking created", "Booking " + event.bookingId() + " has been created.");
        createIfRecipientPresent(event.providerId(), "New booking received", "Booking " + event.bookingId() + " has been created.");
    }

    @EventListener
    public void onPaymentChanged(PaymentStateChangedEvent event) {
        createIfRecipientPresent(event.consumerId(), "Payment state changed", "Payment is now " + event.state() + ".");
    }

    @EventListener
    public void onReviewCreated(ReviewCreatedEvent event) {
        createIfRecipientPresent(event.providerId(), "Review created", "Review " + event.reviewId() + " has been created.");
    }

    private void createIfRecipientPresent(UUID recipientId, String subject, String body) {
        if (recipientId != null) {
            create(recipientId, NotificationChannel.IN_APP, subject, body);
        }
    }
}
