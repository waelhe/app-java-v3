...
> **الغرض:** هذا هو المرجع الحاكم لأهداف مستوى الخدمة (SLO) وقواعد التنبيه المرصودة عليها. يُقرأ مع `monitoring/prometheus-rules/marketplace-alerts.yml` (القواعد ككود-مصدر) وحارسه الاختباري `AlertRulesYamlTest` (يُفشل البناء إن نطقت قاعدة بمقياس غير مُثبت الوجود).
>
...
- **تصدير المقاييس متاح من اليوم:** `/actuator/prometheus` مكشوف في base وprod (`application.yml:135-139` و`application-prod.yml:94-97`)، والصحة على منفذ إدارة منفصل في prod (`:80-82`).
...
| الدين | نقطة الإغلاق |
|---|---|
| لا مثيّر Alertmanager موصولاً — القواعد ككود خاملة حتى التوصيل | قرار جهة التشغيل عند اختيار مستهلك المراقبة (يقترن بالبوابة C — خطة الاستضافة الحاكمة) |
| لا لوحات Grafana — مؤجلة عمداً حتى يوجد المستهلك (لا تُبنى لوحات لأداة غير مختارة) | نفس البوابة |
| لا runbooks تفاعلية للحوادث المذكورة (خطوات «ماذا أفعل عندما يشتغل التنبيه») | المرحلة ج من خارطة الإغلاق (الاسترداد والحوادث) |
| كل العتبات مبدئية بلا ترافيك مرجعي | المعايرة بعد أول canary (خطة الإطلاق) |
| `marketplace.eventbus.stale` حداثتها تتبع إيقاع الفحص لا الزمن الحقيقي | مقبول بحكم أن عتبة التنبيه 5 دقائق ≫ إيقاع الفحص؛ يُعاد النظر إن طُلب أقل |

## 7. دورة حياة السجلات (مكمل تشغيلي من نفس المرحلة)

تدوير أُطري بإعدادات Boot الرسمية في `application.yml:183-190`: 50MB للملف، 14 يوماً احتفاظاً، سقف 2GB إجمالاً، تنقيف الأرشيف عند الإقلاع (`logging.logback.rollingpolicy.*` — الوثيقة الرسمية محفوظة: `scripts/ops-phase-a/boot-logging-official.html`). يعمل مع الخرج المهيكل (`logging.structured.format.file: logstash` — نفس الملف `:176-180`) لأن كليهما يديره Boot على نفس الملحق الملفي.

## 8. بروتوكول الصيانة

- أي تعديل عتبة أو إضافة قاعدة: يُعدّل ملف القواعد + هذا المستند معاً (القيمة ومبررها)، والحارس `AlertRulesYamlTest` هو البوابة — إضافة مقياس جديد تتطلب إثبات وجوده أولاً (file:line) ثم إضافته للقائمة البيضاء في الاختبار.
