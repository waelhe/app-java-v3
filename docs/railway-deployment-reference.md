...
>
> **النشر الحي (آخر تحقق 2026-09-05، جولة #210):** `https://app-java-v3-production-d020.up.railway.app` (بروفايل prod) — الإنتاج يقلع بكود **`8ada038` = #209** (دفعه المستخدم إلى fork النشر): التحقق الحي عمومي — `/actuator/modulith` = 401 (كود #209 موجود ومؤمّن) + liveness/readiness/jwks 200؛ معرّف النشرة غير مقروء هذه الجولة (توكن GraphQL 403 — بوابة التدوير) وفحص health الكامل 503 = البند 3 الموثق (MAIL_* placeholders بيد المستخدم). **main الآن `de49bd0` = #210 (دورة حياة الأحداث: المراقب الرسمي للميتِ الأسماء + إعادة إرسال المسح 15 دقيقة — صفر ترحيلات، إقلاع بنفس الأعمدة V1..V30)** — يُنشَر بدفعه المعتاد عبر fork. (الطبقة السابقة: نشر `e38736b6` من `c072cb9` = #206 — شجرة تشغيل متطابقة بايتاً لـ `6a47e066` = #205؛ إقلاع 11.119s، صفر ترحيلات، صفر ERROR — دفعه 16:20:51Z.)
