...
> **الحالة (محدّثة 2026-09-25T17:12:33Z): نشِط — Railway هو منصة الإنتاج الحية (طبقة التطبيق) على حساب v3 (مشروع `app-java-v3`‏ `c8ba2537` — قناة GitHub مباشرة من `waelhe/app-java-v3@main`، بلا fork)؛ قناة قاعدة البيانات = Neon الخارجية منذ 2026-09-18T01:58Z (القناة المباشرة + `channel_binding=require` — مستعادة 2026-09-19)؛ قناة fork/جيت فروغ والحساب القديم (`optimistic-rebirth`/`-d020`) متقاعَدان — §0.7؛ خطة التكرار عند انتهاء الفترة التجريبية (تجديد/حساب جديد/مزوّد آخر) موثقة في `docs/operating/trial-renewal-and-provider-migration-runbook.md`**
>
> **النشر الحي (آخر تحقق 2026-09-25T17:12:33Z — قناة commit status من تطبيق Railway [السياق `app-java-v3 - app-java-v3` — قناة خطوة freshness نفسها؛ توكن GraphQL غير متاح في جلسة L43 — ملاحظة §15]):** `https://app-java-v3-production.up.railway.app` (بروفايل prod؛ ثلاث خدمات: التطبيق `30294a45` حية، و`app-java-v3-staging` [موجة #373/#375 — Dockerfile مخصص بمعرف cache مستقل، الخدمة `ca6659b8-…`]، و`app-java-v3-edge` `51549c2b` — إخفاقات بنائها الأولى **تاريخية** [2026-09-21، بعد دمج #366 بعقده المُرضى `${PORT:8081}` + probes: ثلاث نشرات 91ث/17ث/33ث، لا بناء مكتمل قط يومئذ، وتسمية الجذر كانت تتطلب سجلات بناء Railway بيد المستخدم]؛ **أحدث نشر مسجل لها أخضر: نجاح 2026-09-25T16:16:50Z على main `24e1e998c6`** [قناة commit status — السياق `app-java-v3 - app-java-v3-edge`]؛ `.railway/railway.ts` خامل وقت النشر رسمياً — «Railway doesn't read `.railway/` during deploys») — **المصدر المُعد للخدمة هو `waelhe/app-java-v3@main` بauto-deploy على كل دمج، وmain يبني الآن بنفسه: PR #347 دُمج (squash `333f565cb7` بكلمة «ادمج» بعد البوابات الثلاث المقيسة لحظة الدمج) ⇒ أول نشر أخضر من main على الحساب: `163d5d79` SUCCESS 2026-09-19T19:20:43Z (~80 ثانية — بناء دافئ بمعرف الـcache-mount المُعاد ربطه) + دخان كامل + أول جولة watchdog مجدولة سليمة (run #109) أغلقت حادثة #346 تلقائياً بخطوة التعافي.****PR #348 دُمج بعده (squash `8a6547eea1` بكلمة «ادمج» 2026-09-19T22:41:12Z — الـrunbook التشغيلي للتكرار والترحيل) ⇒ النشر `fa445f1f` SUCCESS 22:42:03Z (49 ثانية من إنشاء النشر إلى النجاح — دفعة `*.md` صرفة فشجرة التطبيق بالبايت وذاكرة BuildKit كاملة) + دخان كامل ALL PASS + CI على main 4/4 مجموعات نجاح + جولة watchdog ‏#111 سليمة على الرأس الجديد (23:26:20Z).** **PR #349 دُمج بعده (squash `83e7a4b319` بكلمة «ادمج» 2026-09-20T00:34:35Z — truth-sync موجة #348 بالقيمة المقيسة 49.177 ث) ⇒ النشر `91646fc4` SUCCESS 00:35:22Z (45.257 ثانية من إنشاء النشر إلى النجاح — رقم قياسي جديد للحساب؛ دفعة `*.md` صرفة فشجرة التطبيق بالبايت وذاكرة BuildKit كاملة) + دخان كامل ALL PASS + CI على main كل الفحوص نجاح (5/5 مجموعات) + جولة watchdog ‏#112 سليمة على الرأس الجديد (00:50:40Z).** **PR #350 دُمج بعده (squash `a5e4b5cf38` بكلمة «ادمج» 2026-09-20T02:42:27Z — truth-sync موجة #349 + تفعيل CodeRabbit محقق بالقياس) ⇒ النشر `abcbf147` SUCCESS 02:43:12.799Z (43.622 ثانية من إنشاء النشر إلى النجاح — الرقم القياسي الثالث توالياً؛ دفعة `*.md` صرفة فشجرة التطبيق بالبايت وذاكرة BuildKit كاملة) + نبض إقلاع 67 ترحيلة/23.856 ث/صفر أخطاء + دخان كامل ALL PASS + CI على main كل الفحوص نجاح + جولة watchdog ‏#114 سليمة على الرأس الجديد (03:03:28Z).** **PR #351 دُمج بعده (squash `ec4179ce7c9e` بكلمة «ادمج» 2026-09-20T05:19:03Z — truth-sync موجة #350) ⇒ النشر `13e1d4cc` SUCCESS 05:19:49.444Z (43.944 ثانية من إنشاء النشر إلى النجاح — دفعة `*.md` صرفة فشجرة التطبيق بالبايت وذاكرة BuildKit كاملة) + نبض إقلاع 67 ترحيلة/24.364 ث/صفر أخطاء + دخان كامل ALL PASS (8/8) + CI على main كل الفحوص نجاح (4/4 مجموعات) + جولة watchdog ‏#115 سليمة على الرأس الجديد (06:09:32Z).** **PR #352 دُمج بعده (squash `de340bc5928c` بكلمة «ادمجه» 2026-09-20T07:48Z — الموجة التوثيقية الطرفية: truth-sync موجة #351 + اعتماد ترويسة CodeRabbit ج1 بكوميت `08d34d0`) ⇒ النشر `13ddd7a9` SUCCESS 07:49:33.235Z (61.520 ثانية من إنشاء النشر إلى النجاح — ذاكرة BuildKit أبرد بعد نافذة خمول؛ الدفعة `*.md` صرفة فشجرة التطبيق بالبايت) + نبض إقلاع 67 ترحيلة/35.099 ث/صفر أخطاء + دخان كامل ALL PASS (8/8) + جولة watchdog ‏#117 سليمة على الرأس الجديد (07:57:49Z).** **PR #353 دُمج بعده (squash `3d1fde3194` بكلمة «ادمج» 2026-09-20T11:23:03Z — طبقة L44 الوظيفية: الرسائل المباشرة + V67 + أول تطبيق للقاعدة المصححة [التوثيق داخل الـPR الوظيفي]) ⇒ النشر `729a7997` SUCCESS 11:26:36Z (مقيسًا عبر قناة commit status من تطبيق Railway [السياق `app-java-v3 - app-java-v3` — قناة خطوة freshness نفسها]: pending أُنشئت 11:23:08Z فالتقط auto-deploy الرأس خلال ~5 ثوانٍ ⇒ نجاح بعد ~3د28ث — بناء وظيفي دافئ: إعادة بناء Maven + تطبيق V67) + دخان كامل ALL PASS **بـ108 مسارات** (سطح L44 المسجّل + عقد 401 problem+json عليه) + جولتا watchdog ‏#118/#119 سليمتان على الرأس الجديد + CI على main كل الفحوص نجاح (اكتملت 12:05Z) — قياس §15.** **PR #354 دُمج بعده (squash `c9a32f7c1a78` بكلمة «ادمج» 2026-09-20T15:10:47Z — طبقة L43 الوظيفية: فئة التوصيات + زوج V68/V69 + جولة CI ج1 فشلت بالفحصين الإلزاميين فأُصلح جذرها بكوميت `6ac69ca` [الاختبار يستهلك صفر تصاريح postCreate]) ⇒ النشر `ed8e441c` SUCCESS 15:14:08Z (مقيسًا عبر قناة commit status: pending أُنشئت 15:10:51Z فالتقط auto-deploy الرأس خلال ~4 ثوانٍ ⇒ نجاح بعد ~3د17ث — بناء وظيفي دافئ: إعادة بناء Maven + تطبيق زوج V68/V69 على Neon فالسجل الحي 70 مدخلاً/الفهرس V69) + دخان كامل ALL PASS ‏10/10 **بـ108 مسارات ومفردة Swagger الحية تحمل القيمة الرابعة** (مرشح التغذية + مخطط الإنشاء بدلالة L43) + عقد سطح التصفية 401 problem+json + جولة watchdog ‏#121 سليمة على الرأس الجديد (15:17:49Z) + CI على main كل الفحوص نجاح — قياس §15.** **PR #356 دُمج بعده (squash `fb61f5b4c0d9` بكلمة «ادمج» 2026-09-20T22:30:18Z — وحدة edge الوظيفية: بوابة BFF standalone خارج حدود Modulith بتصميم [Spring Cloud Gateway Server MVC + عميل OAuth2 سري `edge` + D6 + TokenRelay + جلسات Redis + csrf.spa] — المفاعل صار 22 وحدة لكن شجرة التطبيق غير مماسة: الـDockerfile يبني `-pl marketplace-app -am` وedge ليست اعتماداً للـapp [الـDockerfile متطابق حرفياً بين main والفرع] فالوحدة تُبنى وتُختبر في CI فقط ولها نشر كخدمة = بوابة قرار بيد المستخدم §14.3) ⇒ النشر `bfd0633a` SUCCESS 22:32:29Z (حالة commit status من تطبيق Railway — ~2د11ث من الدمج إلى الخضراء، بناء دافئ بشجرة تطبيق مكافئة) + دخان المحطة الختامية ALL PASS ‏15/15 بـ108 مسارات (النظام + أسطح المرحلة الثانية كاملة L41/L43/L44/L45 بعقود 401 problem+json + مفردة Swagger الحية بالقيمة الرابعة) + CI على main كل الفحوص نجاح (Full Integration ~23:06Z + Build & Test ~23:11Z) + جولتا watchdog على الرأس الجديد كلتاهما نجاح (المجدولة 23:02:59Z + dispatch 23:09:00Z) — قياس §15.**  **PR #366 دُمج بعده (الدمج الاستثنائي — squash `c94866c5d383` بكلمة المستخدم «فعل المراجعات CodeRabbit على ٣٦٦ فقط وادمج بشكل استثنائي بدون انتظار حالة ci على الpr. لان المهم حالته على النشر» 2026-09-21T21:29:12Z — عقد فحص الصحة للبوابة: `server.port: ${PORT:8081}` + `management.endpoint.health.probes.enabled: true` في yml الـedge + تثبيت `PORT: "8081"` في IaC [+30/-1، ملفان]؛ CodeRabbit فُعِّل على #366 وحده بالتعليق الرسمي فحكم «Review completed» + «No actionable comments» بخيوط 0/0؛ والدمج أُطلق دون أي انتظار CI فهبط في المحاولة الأولى — الفحصان المطلوبان اكتملا طبيعياً 21:25:37Z/21:27:55Z خلال نافذة التفعيل والمنصة `bypass=never` مقيسة]) ⇒ دورة التطبيق خضراء كاملة: النشر `1777df03` SUCCESS 21:31:20Z (~2د04ث دافئ) + دخان المحطة ALL PASS ‏16/16 بـ108 مسارات + CI على main كل الفحوص الخمسة نجاح (Full Integration 22:07:41Z، Build & Test 22:09:19Z) + جولة watchdog dispatch ‏#137 سليمة (21:40:32Z). **نشر edge والحقيقة المصححة:** النشر `2e039a1d` فشل بعد 33ث (pending 21:29:16Z → failure 21:29:49Z)؛ قائمة الحالات الخام تُثبت أن `9438ba0f` (موجة #364) عاش **17 ثانية** لا «~10 دقائق» — الفجوة كانت بين فشل المحاولة الأولى وبدء إعادة المحاولة؛ **لا بناء edge اكتمل قط** (91ث/17ث/33ث) ورواية «البناء البارد يكتمل والفحص يسقطه حتى 300ث» **مُسحوبة** كقراءة توقيت خاطئة — عيبا PORT/probes (الحقيقيان الكامنان) أصلحهما #366 لكن لم يكونا حاجب هذه النشرات؛ **الحاجب الحي:** البناء يموت مبكراً والجهة المضمونة سليمة (CI ‏6/6 على نفس الشجرة + الوصفة مطابقة لوصفة التطبيق + صيغة cache-mount مطابقة للدليل الرسمي) — **تسمية الجذر تتطلب سجلات بناء Railway للنشر `2e039a1d`** (قناة المستخدم؛ بها سُمّي جذر cacheKey سابقاً)؛ وقياس رسمي إضافي: «Railway doesn't read `.railway/` during deploys» (وثائق IaC المجلوبة حياً) — تثبيت PORT في railway.ts لا يعمل وقت النشر (يتطلب `railway config apply` أو وصفة railwayapp/config — بيد المستخدم) بينما `${PORT:8081}` في yml يعمل بمعزل عنه — قياس §15.** **PR #364 دُمج بعده (squash `2f19758d7c41` بكلمة المستخدم «ادمج 364» 2026-09-21T20:01:10Z — استبدال cache-id البديل `edge-placeholder` بمعرف خدمة edge الحقيقي `51549c2b-…` في `marketplace-edge/Dockerfile` [البوابات الثلاث مقيسة حياً لحظة الدمج: CI ‏6/6 على `bfc1daac` + CodeRabbit نجاح + خيوط 0/0 + mergeable/CLEAN]) ⇒ دورة التطبيق خضراء كاملة: النشر `ac509319` SUCCESS 20:03:34Z (~2د19ث دافئ) + دخان المحطة ALL PASS ‏16/16 بـ108 مسارات + CI على main كل الفحوص نجاح (Build & Test 20:37:22Z، Full Integration 20:40:23Z) + جولة watchdog dispatch ‏#136 سليمة (20:08:37Z). **نشر edge بعد الإصلاح — نشران فاشلان بتوقيعين:** `e9985b9f` (~91ث: الفشل انتقل داخل البناء بعد زوال رفض cacheKey؛ تزامن مع بناء التطبيق المتوازي) ثم `9438ba0f` (إعادة محاولة Railway، **~10 دقائق**: البادئ البارد يكتمل والحاوية تقلع وفحص الصحة يفشل حتى مهلة 300ث) — **الجذر مشخَّص بالأسناد الرسمية** (docs.railway.com/deployments/healthchecks جُلبت حياً 2026-09-22): خرق عقد PORT («This variable's value is also used when performing health checks» — edge يصلب 8081 والتطبيق يلتزم `${PORT:8080}`) + probes غائبة (`/actuator/health/liveness` → 404)؛ **الإصلاح في PR #366** (`${PORT:8081}` + `probes.enabled: true` + تثبيت `PORT: "8081"` في IaC) بانتظار كلمة المستخدم §14.3 ثم تجسيد قيم EDGE_*/REDIS_*/AUTH_SERVER_ISSUER بيد المستخدم (issuer-uri يُحل عند الإقلاع عبر OIDC discovery — توثيق المستودع) — قياس §15.** **PR #363 دُمج بعده (squash `ea63919179` بكلمة المستخدم «راجع pr 363 وادمج» 2026-09-21T19:09:14Z — سجل حقيقة موجة #360 في الملفات الثلاث [دفعة `*.md` صرفة +20/-3؛ البوابات الثلاث خضراء من أول قياس: CI ‏6/6 على `8d87fd3` + CodeRabbit نجاح بتخطٍّ توثيقي وخيوط 0/0 + mergeable/CLEAN]) ⇒ النشر `e02749be` SUCCESS 19:10:55Z (pending 19:09:20Z — ~1د35ث، بناء دافئ) + دخان المحطة ALL PASS ‏16/16 بـ108 مسارات + CI على main كل الفحوص نجاح (Full Integration 19:47:46Z، Build & Test 19:49:17Z) + جولة watchdog dispatch ‏#135 سليمة على الرأس الجديد (19:17:29Z). **بالتزامن حقيقة تشغيلية جديدة:** خدمة `app-java-v3-edge` (`51549c2b`) مُنشأة على Railway منذ ~18:39Z ونشرها يفشل فوراً بـ«missing cacheKey prefix» (البديل `edge-placeholder` في `marketplace-edge/Dockerfile` صار حاجباً حياً لحظة نشوء الخدمة — نشران مقيسان فاشلان `ef2315cf`/`5c4a456a`؛ جسم PR #364 يوثق 3 نشرات بتحقق المستخدم من سجلات Railway JSON) — **PR #364 مفتوح** (رأس `bfc1daac`، سطر واحد: استبدال البديل بمعرف الخدمة) بانتظار كلمة دمج المستخدم §14.3، ودونه تبقى خدمة edge معطلة النشر بينما خدمة التطبيق سليمة تماماً — قياس §15.** **PR #360 دُمج بعده (squash `f41ef1b95360` بكلمة المستخدم «افحص وادمج حسب المتبع» 2026-09-21T15:14:56Z — موجتان: متانة جلسات Redis الرسمية [إنتاج Upstash مُدار، قرار الطبقة الحرة: لا نسخ احتياطية، الخطر مقبول وموثق، الإخلاء معطّل؛ AOF+RDB على compose المملوك؛ جلسات JDBC مرفوضة] + تعريف نشر البوابة كخدمة ثانية [`marketplace-edge/Dockerfile` بوصفة الجذر + IaC خدمة `app-java-v3-edge` في railway.ts بمصدر github@main وباني DOCKERFILE وliveness ب300ث وpreserve() لكل المتغيرات] — **الخدمة معرّفة على main لكن غير منشأة على Railway بعد**: التطبيق الفعلي وقيم EDGE_* واستبدال cache-id الظرفي خطوات تشغيلية تالية بيد المستخدم؛ Dockerfile التطبيق خارج الفرق فصورة الإنتاج لم تتغير؛ خيوط CodeRabbit ‏7/7 محلولة عبر ثلاث جولات) ⇒ النشر `b4b9074` SUCCESS 15:17:32Z (pending 15:15:01Z — ~2د31ث، بناء دافئ) + دخان المحطة ALL PASS ‏16/16 بـ108 مسارات + CI على main كل الفحوص نجاح (Full Integration 15:54:17Z، Build & Test 15:55:11Z) + جولة watchdog dispatch ‏#133 سليمة على الرأس الجديد (15:18:07Z) — قياس §15.** **PR #361 دُمج بعده (squash `a41bd8f90058` بكلمة المستخدم الشرطية «اذا كان كل شيء سليم ومكتمل ادمجه حسب المتبع» 2026-09-21T10:50Z — تحديث لقطة الرسم المعرفي إلى جيل 6f70886f [4050 عقدة/5582 حافة]: دفعة `.ua/` صرفة تدخل سياق البناء لكن لا شيء منها يدخل صورة runtime فمصنوعات التشغيل متطابقة؛ مراجعات CodeRabbit مفعّلة ومكتملة بالقياس [Review finished + 0/0 خيوط]) ⇒ النشر `da2f8be2` SUCCESS 10:52:19Z (pending 10:50:20Z — ~1د49ث من الدمج إلى الخضراء، بناء دافئ) + دخان المحطة ALL PASS ‏16/16 بـ108 مسارات + CI على main كل الفحوص نجاح (Build & Test 11:27:01Z، Full Integration 11:29:28Z) + جولتا watchdog dispatch على الرأس الجديد كلتاهما نجاح (#129/#130) — قياس §15.** (قبل الدمج كان النشر اليدوي `4b203ec2` يحمل `d43840d` عبر `serviceInstanceDeployV2` لأن main القديم لا يستطيع البناء — قصة الانتقال كاملة في §0.7 و§15.) **PR #383 دُمج بعده (squash `24e1e998c6` بأمر المستخدم «نفذ...» 2026-09-25T16:14:15Z — بوابة CVE تبني طازجًا: `no-cache: true` + `pull: true` بالمدخلات الرسمية لـdocker/build-push-action بعد قياس تجمّد حزم OS بطبقة الـcache [expat ‏2.8.5-r0 في فهرس v3.24 منذ 2026-09-23T22:37Z والمسح يصادق 2.8.4-r0] + اعتماد ملاحظة CodeRabbit ج1 من الجذر بكوميت `a06ff47`: إزالة `cache-from`/`cache-to` الميتين — «BuildKit doesn't preserve cache mounts in the GitHub Actions cache by default» — وحارس `doesNotContain` يمنع عودتهما) ⇒ الخدمات الثلاث خضراء (قناة commit status): staging ‏16:15:23Z (~68ث) + الإنتاج نجح 16:16:36Z (**~2د21ث** — ملف الاختبار يمس سياق البناء فأُعيد بناء Maven بذاكرة .m2 دافئة) + edge ‏16:16:50Z + دخان ALL PASS ‏6/6 بـ108 مسارات + CI على main كل الفحوص نجاح + جولة watchdog #163 سليمة (16:18:05Z) + **سعر المسح الطازج المقيس: 184ث/201ث** (main/الفرع — ضمن ميزانية 45د) — قياس §15.** — **قناة قاعدة البيانات: Neon الخارجية** (`ep-weathered-pond-b2vgv0jc.c-6.eu-central-1.aws.neon.tech/neondb` — القناة المباشرة لا pooler) — **سجل Flyway الحي عليها: 70 مدخلًا (68 إصدارية + بذرتا R__)، الفهرس V69** (وصول L43 مقيس بالنشر `ed8e441c` SUCCESS 15:14:08Z — نجاح النشر يقتضي إقلاعاً يمر بFlyway migrate [ddl-auto: none] فأي فشل في زوج V68/V69 كان سيفشل النشر؛ به اكتملت المرحلة الثانية للخطة المجتمعية). الدخان الحي (أحدث قياس 2026-09-25T16:25Z على الرأس `24e1e998c6`): liveness/readiness UP + jwks `kid=marketplace-jwt-r3` + OIDC 200 (issuer بالنطاق الحي) + `/actuator/modulith` 401 + **108 مسارات في `/v3/api-docs`** (فيها أسطح L44 وL45 وسطح L43 الحي: مرشح التغذية `category=RECOMMENDATION` بعقد 401 problem+json ومفردة Swagger الحية تحمل القيمة الرابعة) + عقود 401 problem+json على أسطح المرحلة الثانية كاملة — ALL PASS ‏6/6 (محطة هذه الموجة؛ التعداد الكامل 15/15 آخر قياس له بعد دمج #356). **النقل الصادق للمتغيرات (2026-09-19):** 45 متغير تطبيق بالضبط من الحساب القديم كما هي عدا الثلاث الحاملة للنطاق (issuer/CORS/BFF-redirect) و`DB_PASSWORD` المدوّرة (القديمة ميتة مقيسًا 28P01×3) — القياس الكامل في SYSTEM.md §15 (2026-09-19). **بدون دمج PR #347 كان يفشل أي بناء من main بتحقق BUILD_IMAGE نفسه — ✅ دُمج 2026-09-19T19:19Z (squash `333f565cb7`) وأول بناء من main أخضر (`163d5d79`).**
