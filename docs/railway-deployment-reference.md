...
>
> **النشر الحي (آخر تحقق 2026-09-05، جولة TTL المخبآت):** `https://app-java-v3-production-d020.up.railway.app` (بروفايل prod) — fork النشر = **`78df7f8` = #212** (إزالة متجر Quartz الميت/V31 — دفعه المستخدم بيده 2026-09-04 20:52:30Z، تحقق GitHub API العام؛ fork = upstream بالبايت؛ عند الإقلاع طبّق Flyway V31) والقنوات العمومية خضراء بعده (liveness/readiness/jwks/OIDC 200 + `/actuator/modulith` = 401 — تحقق مباشر 2026-09-05) — معرّف النشرة وسجلاتها غير مقروءة (توكن GraphQL 403 — بوابة التدوير) وفحص health الكامل 503 = البند 3 الموثق (MAIL_* placeholders بيد المستخدم). **main يتقدم بطبقة TTL المخبآت** (`spring.cache.redis.time-to-live: 1h` — كل صف مخبأة redis ينتهي ذاتيًا؛ الافتراض الرسمي كان «لا انتهاء أبدًا») — يُنشَر بدفعه المعتاد عبر fork. (الطبقة السابقة: `de49bd0` = #210 دفعه 19:44:58Z — القنوات خضراء.)
