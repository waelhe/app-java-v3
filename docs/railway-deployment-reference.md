...
> **الحالة (محدّثة 2026-09-19T17:43Z): نشِط — Railway هو منصة الإنتاج الحية (طبقة التطبيق) على حساب v3 الجديد (مشروع `app-java-v3`‏ `c8ba2537` — قناة GitHub مباشرة من `waelhe/app-java-v3@main`، بلا fork)؛ قناة قاعدة البيانات = Neon الخارجية منذ 2026-09-18T01:58Z (القناة المباشرة + `channel_binding=require` — مستعادة 2026-09-19)؛ قناة fork/جيت فروغ والحساب القديم (`optimistic-rebirth`/`-d020`) متقاعَدان — §0.7**
>
> **النشر الحي (آخر تحقق 2026-09-19T17:43Z — قناة GraphQL v2 مقروءة):** `https://app-java-v3-production.up.railway.app` (بروفايل prod، خدمة واحدة `30294a45`) — **المصدر المُعد للخدمة هو `waelhe/app-java-v3@main` بauto-deploy على كل دمج** (قياس: بناء تلقائي عند دمج #345 قبل الهجرة)؛ **والنشرة الحاملة الحالية `4b203ec2` SUCCESS 2026-09-19T17:42:11Z تحمل `d43840d`** — **رأس فرع fix/deploy-v3-cache-id (PR #347)، نُشر يدويًا عبر `serviceInstanceDeployV2(commitSha)`** لأن main قبل الدمج لا يستطيع البناء (معرف الـcache-mount فيه ما زال القديم — قياس status «Deployment failed» على رأس main)؛ بعد الدمج يستأنف main البناء التلقائي بنفسه. (إقلاع 23.155s؛ `Successfully validated 67 migrations (00:00.614s)` بهوية `neondb_owner` المستعادة؛ سطر الاتصال الحرفي `Database: jdbc:postgresql://ep-weathered-pond-b2vgv0jc.c-6.eu-central-1.aws.neon.tech/neondb?channel_binding=require&sslmode=require (PostgreSQL 18.6)`؛ صفر أسطر ERROR) — **قناة قاعدة البيانات: Neon الخارجية** (`ep-weathered-pond-b2vgv0jc.c-6.eu-central-1.aws.neon.tech/neondb` — القناة المباشرة لا pooler) — **سجل Flyway الحي عليها: 67 مدخلًا (65 إصدارية + بذرتا R__)، الفهرس V66**. الدخان الحي 2026-09-19T17:43Z: liveness/readiness UP + jwks `kid=marketplace-jwt-r3` + OIDC 200 (issuer بالنطاق الجديد) + `/actuator/modulith` 401 + 107 مسارًا في `/v3/api-docs` + عقد 401 problem+json. **النقل الصادق للمتغيرات (2026-09-19):** 45 متغير تطبيق بالضبط من الحساب القديم كما هي عدا الثلاث الحاملة للنطاق (issuer/CORS/BFF-redirect) و`DB_PASSWORD` المدوّرة (القديمة ميتة مقيسًا 28P01×3) — القياس الكامل في SYSTEM.md §15 (2026-09-19). **بدون دمج PR #347 يفشل أي بناء من main بتحقق BUILD_IMAGE نفسه (المعرف في main ما زال القديم).**
...
| النتيجة الحية | `4b203ec2` SUCCESS 17:42:11Z تحمل `d43840d`: إقلاع 23.155s + `Successfully validated 67 migrations` بهوية `neondb_owner` + صفر أسطر ERROR + دخان كامل (readiness UP على قناة DB المباشرة، jwks r3، OIDC بالنطاق الجديد، 107 مسارات) | سجلات النشرة عبر GraphQL v2 + فحص curl مباشر |

**المتبقي بيد المستخدم حصرًا:** دمج PR #347 (بدونه يبقى البناء من main فاشلًا بنفس التحقق) — ثم إبطال مفتاح Gemini المفصوح والدوران المستحقان الموثقان في §15/PROJECT_MAP.
