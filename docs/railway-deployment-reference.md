...
> **الحالة (محدّثة 2026-09-05): نشِط — Railway هو منصة الإنتاج الحية**
>
> **النشر الحي (آخر تحقق 2026-09-05، جولة إزالة Quartz):** `https://app-java-v3-production-d020.up.railway.app` (بروفايل prod) — fork النشر = **`de49bd0` = #210** (دفعه المستخدم بيده 2026-09-04 19:44:58Z — تحقق GitHub API العام؛ البناء التلقائي بحسب سلوك المنصة الموثق) والقنوات العمومية خضراء بعده (liveness/readiness/jwks 200 + `/actuator/modulith` = 401) — معرّف النشرة وسجلاتها غير مقروءة (توكن GraphQL 403 — بوابة التدوير) وفحص health الكامل 503 = البند 3 الموثق (MAIL_* placeholders بيد المستخدم). **main يتقدم بحزمة إزالة آلة Quartz الميتة (V31: إسقاط جداول QRTZ الإحدا عشر + إزالة الاعتمادية والكتل — الجدولة على مجدول Spring حصراً)** — يُنشَر بدفعه المعتاد عبر fork. (الطبقات السابقة: نشر `e38736b6` من `c072cb9` = #206 — شجرة تشغيل متطابقة بايتاً لـ `6a47e066` = #205؛ إقلاع 11.119s، صفر ترحيلات، صفر ERROR — دفعه 16:20:51Z.)
