...
- **التاريخ:** 2026-09-07.
- **الوضع:** مرجع موافقة. لا يُنفَّذ أي تعديل كود/ترحيل/فرع قبل اعتماد المستخدم لهذه الخطة.
...
## 7. دفعات التنفيذ (بعتمد بعد الموافقة)

```
Batch A (أمن + فضاء المعرّف):  A1→A2→A3→A4→A5→A6
Batch B (دومين/API):            B1→B2  (+ B3/B4/B5 بعد قرار المستخدم)
