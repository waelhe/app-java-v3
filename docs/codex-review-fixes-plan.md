...
- **التاريخ:** 2026-09-07.
- **الوضع:** معتمدة ومنفَّذة جزئياً — Batch A (A1→A6) و B1/B2 من Batch B و C1/C2 من Batch C منفَّذة عبر PR #253 (فرع `feat/codex-still-valid-sweep`، مفتوح قيد المراجعة)، بما فيها جولات إصلاح ملاحظات CodeRabbit (تحدي WWW-Authenticate وفق RFC 6750 §3/§3.1، وتأكيد `senderId` في اختبار WebSocket) وإصلاح عيوب تغليف سكربتات C2 على رأس PR المحدَّث (BOM قبل الـshebang يمنع التنفيذ المباشر، صلاحية 644 بدل 755 فتفشل حالة «مغطى → خروج 0» فعلياً على لينكس، وسطر نهاية ملف غائب). C3 من Batch C ينتظر التنفيذ، وبنود DECISION (A7/A8/B3/B4/B5) تنتظر قرار المستخدم — لا يُنفَّذ منها شيء قبل القرار.
...
Batch C (بنية/CI):              C1→C2 — **منفَّذ عبر PR #253** (C2 مُكمَّلة بإصلاح تغليف: إزالة BOM قبل الـshebang، صلاحية 755 للسكربتين الجديدتين، وسطر نهاية الملف — لتصبح حالة الاختبار 2+ قابلة للتنفيذ فعلياً)؛ C3 ينتظر، وC4 قرار موثق
```
