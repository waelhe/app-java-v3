...
## 4. البنود القرارية (DECISION — اعتمدها المستخدم 2026-09-07 ونُفِّذت كما مُوصى بها)

> **حالة التنفيذ (2026-09-07):** اعتماد متزامن «اعتمد الكل كما هو معروض (موصى به)» لكل البنود الخمسة، ثم تنفيذ كل توصية كما هي — في جلسة ثم PR واحد بالحوكمة الكاملة (رقمه #255 — مفتوح بانتظار الدمج). المراجع المؤكدة لكل قرار: A7 — `OAuth2ClientSecretInitializer.java:224-226` (900s/604800s، وSPA بلا refresh في `OAuth2PublicClientInitializer.java:182,187`) + `ExpiredAuthorizationsCleanup.java:106,:65` (تنظيف يومي 04:00 UTC واحتفاظ 7 أيام) + سطر طوارئ في `docs/observability/runbooks.md` §1.3. A8 — Javadoc مطبَّق في `SecurityConfig.java:499-517` + اختبارا `SecurityConfigJwtDecoderTest` (قبول aud يتضمّن المتوقع، رفض قائمة بلا المتوقع). B3 — `BookingInfo.java:19-20` `priceCents < 0` فقط (نعم `0` = مجاني، مطابق `V2:8`) + `BookingInfoTest` حالتان. B4 — `AvailabilityPort.hasExactAvailableSlot` + بوابة مبكرة في `BookingService.create` (400: «The requested time window must match an available slot exactly») + اختبارات الوحدة والبوابة. B5 — `V41__payment_webhook_provider_scoped_dedup.sql` (فهرس مركب وحيد: UNIQUE(provider,event_id) بدل UNIQUE(event_id)) + بحث/مسح provider-scoped في `PaymentsService`/`WebhookEventRecorder` + اختبار إقصاء «نفس event_id بقناتين ≠ dedup». التحقق: 318 اختبار وحدة للوحدات المتأثرة خضراء (`mvn -pl marketplace-shared,marketplace-availability,marketplace-booking,marketplace-payments,marketplace-platform-infra -am test`).
