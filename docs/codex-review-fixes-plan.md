...
- **التاريخ:** 2026-09-07.
- **الوضع:** معتمدة ومنفَّذة جزئياً — Batch A (A1→A6) و B1/B2 من Batch B منفَّذة عبر PR #253 (فرع `feat/codex-still-valid-sweep`، مفتوح قيد المراجعة)، بما فيها جولة إصلاح ملاحظات CodeRabbit على تحدي WWW-Authenticate وفق RFC 6750 §3/§3.1. بنود Batch C تنتظر التنفيذ، وبنود DECISION (A7/A8/B3/B4/B5) تنتظر قرار المستخدم — لا يُنفَّذ منها شيء قبل القرار.
...
## 7. دفعات التنفيذ (Batch A + B1/B2 منفَّذة عبر PR #253)
...
Batch C (بنية/CI):              C1→C2→C3   (C4 قرار موثق)
```
