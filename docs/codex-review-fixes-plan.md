...
- **التاريخ:** 2026-09-07.
- **الوضع:** معتمدة ومنفَّذة جزئياً — Batch A (A1→A6) و B1/B2 من Batch B و عبر PR #253 (فرع `feat/codex-still-valid-sweep`، مفتوح قيد المراجعة)، بما فيها جولات إصلاح ملاحظات CodeRabbit (تحدي WWW-Authenticate وفق RFC 6750 §3/§3.1، تأكيد `senderId` في اختبار WebSocket، وجولة ج3 على سكربتات C2: تقويم صلاحية الاستثناء حقيقياً (أطوال الشهور والسنوات الكبيسة) + إغلاق فجوة تغييرات المعاملات/الطلب خارج Return Type بالفشل المقفول [UNREPRESENTABLE] وفق دلالات نموذج openapi-diff الرسمي) وجولة ج4 الأمنية على V39: حذف الإسناد بـ`display_name` كليًا (CWE-639 — `users.display_name` يُعاد كتابته من OIDC `name` claim عند كل دخول عبر `UserService.syncFromOidc`، ولا UNIQUE عليه في V1؛ `sub` وحده المعرف المستقر وفق OIDC Core §5.1) وإصلاح عيوب تغليف سكربتات C2 على رأس PR المحدَّث (BOM قبل الـshebang يمنع التنفيذ المباشر، صلاحية 644 بدل 755 فتفشل حالة «مغطى → خروج 0» فعلياً على لينكس، وسطر نهاية ملف غائب). **Batch C كاملة (C1→C2→C3) منفَّذة عبر PR #253**: C1 = `OpenApiConfig` + `OpenApiConfigTest`، C2 = السكربتات الثلاثة، C3 = `V39__repair_provider_user_id_backfill.sql` + `ProviderUserIdBackfillIntegrationTest` بالصيغة النهائية بعد ج4: **قطع الارتباط بالمحذوفين ناعميًا فقط دون أي إسناد بالاسم** — الصفوف بلا دليل ملكية مستقر تبقى NULL للتدبير اليدوي (التحقق: `mvn -pl marketplace-app -am test -Dtest=ProviderUserIdBackfillIntegrationTest` = 3/3 خضراء على Testcontainers Postgres + Redis)، مع **توثيق قاعدة C4** («لا تعديل ترحيلات مطبقة V{n}») في هذا المستند. بنود DECISION (A7/A8/B3/B4/B5) **اعتمدها المستخدم «اعتمد الكل كما هو معروض (موصى به)»** في 2026-09-07 ونُفِّذت كما مُوصى بها (A7/A8 قبول توثيقي، B3 السماح `0`، B4 Option M، B5 تبنٍّ provider-scoped) — حالة التنفيذ في §4.
...
| B3 | `BookingInfo.java:18-26`, `V2__catalog.sql:8`, `ServiceInput.java` (توحيد عقد السعر: `@Min(0))` | — | ✅ |
...
- **الإصلاح:** ترحيل **جديد** V39: قطع أي ربط إلى مستخدم محذوف ناعميًا (set null) **فقط — دون أي إسناد بالاسم**. **لا تعديل V23** (ترحيل مطبق — ممنوع إعادة الكتابة، انظر C4).
- **تعديل ج4 (CodeRabbit — CWE-639):** الصيغة الأولى من V39 كانت تعيد الربط بالـbackfill المصحح (`is_deleted=false` في join وCOUNT). ملاحظة CodeRabbit الأمنية (Major) صحّتها: `users.display_name` **ليس دليل ملكية** — `UserService.syncFromOidc` يعيد كتابته من OIDC `name` claim عند كل دخول (OIDC Core §5.1: `name` عرضٌ قابل للتغيير و`sub` وحده المعرف المستقر — https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims)، ولا قيد UNIQUE عليه في V1؛ فإسناد ملف بالاسم قد يربطه بمزوّد خاطئ نشط، و`ProviderRepository.findByUserId` (بِرم الملكية «me» L20) يعتبر الإسناد ملكًا حقيقيًا = تجاوز صلاحيات عبر مفتاح يتحكم به المستخدم (CWE-639 — https://cwe.mitre.org/data/definitions/639.html). الصفوف بلا دليل ملكية غير قابل للتغيير تبقى NULL للتدبير اليدوي.
- **الاختبار:** `ProviderUserIdBackfillIntegrationTest` (3 حالات: لا إسناد بالاسم — الصف يبقى NULL؛ قطع الارتباط بالمحذوف ناعميًا؛ ثابت عدم وجود ربط بأي محذوف بعد الترحيل) على Testcontainers Postgres حقيقي مع `ddl-auto=none` وتنفيذ نص الترحيل نفسه حرفيًا.
- **التصنيف:** VIOLATION.

---

## 4. البنود القرارية (DECISION — اعتمدها المستخدم 2026-09-07 ونُفِّذت كما مُوصى بها: ⏳ منفَّذة على فرع PR #255 — بانتظار الدمج)

> **حالة التنفيذ (2026-09-07، محدَّثة):** اعتماد متزامن «اعتمد الكل كما هو معروض (موصى به)» لكل البنود الخمسة، ثم تنفيذ كل توصية كما هي — في جلسة ثم PR واحد بالحوكمة الكاملة. الحالة القانونية الموحدة عبر السجلات الثلاثة (هذا الملف + PROJECT_MAP + SYSTEM.md §11): ⏳ منفَّذة على فرع PR #255 — بانتظار الدمج. المراجع المؤكدة لكل قرار: A7 — `OAuth2ClientSecretInitializer.java:224-226` (900s/604800s، وSPA بلا refresh في `OAuth2PublicClientInitializer.java:182,187`) + `ExpiredAuthorizationsCleanup.java:106,:65` (تنظيف يومي 04:00 UTC واحتفاظ 7 أيام) + سطر طوارئ في `docs/observability/runbooks.md` §1.3. A8 — Javadoc مطبَّق في `SecurityConfig.java:499-517` + اختبارا `SecurityConfigJwtDecoderTest` (قبول aud يتضمّن المتوقع، رفض قائمة بلا المتوقع). B3 — `BookingInfo.java:19-20` `priceCents < 0` فقط (نعم `0` = مجاني، مطابق `V2:8`) + `BookingInfoTest` حالتان. **توحيد عقد السعر عبر الأسطح (مراجعة 2026-09-08، دُقّق بعد ج3 CodeRabbit):** `ServiceInput.java:21` كان `@NotNull @Positive` — GraphQL كان السطح الوحيد الذي يُعلن رفض الإعلان المجاني على مستوى التعليقات التوضيحية (REST `@NotNull` فقط، `V2:8` `>= 0`، `BookingInfo` يرفض `< 0` فقط)؛ صار `@NotNull @Min(0)` (سابقة مبالغ القروش نفسها: `CurrencyExchangeController` `amountCents` `@Min(0)`) + `ServiceInputValidationTest` (4 حالات عبر مزوّد الإطار Hibernate Validator 9.1.3 — صفر/موجب صالحان، سالب/فارغ مرفوضان) — **التناقض على مستوى العقد أُغلق (كل الأسطح تُعلن القاعدة نفسها: الصفر مسموح والسالب مرفوض)؛ سطح GraphQL end-to-end يبقى خلف الدين الموثق أدناه** (الوصلة ترفض كل الأسعار أصلًا)، واختبار الـ mutation بصفر محجوز لما بعد اعتماد إصلاح الوصلة (لا يُضاف اختبار أحمر عمدًا). B4 — `AvailabilityPort.hasExactAvailableSlot` + بوابة مبكرة في `BookingService.create` (400: «The requested time window must match an available slot exactly») + اختبارات الوحدة والبوابة. B5 — `V42__payment_webhook_provider_scoped_dedup.sql` (فهرس مركب وحيد: UNIQUE(provider,event_id) بدل UNIQUE(event_id)) + بحث/مسح provider-scoped في `PaymentsService`/`WebhookEventRecorder` + اختبار إقصاء «نفس event_id بقناتين ≠ dedup». التحقق: 318 اختبار وحدة للوحدات الخمس المتأثرة عند الرأس الأصلي 47a9dfc، و321 عند الرأس المدموج مع main (fa97618: +1 booking من اختبار L26 و+2 availability من اختبارات L27 — اختبارات main المدموجة، صفر تغيير على اختبارات القرارات الخمسة) — الأمر نفسه `mvn -pl marketplace-shared,marketplace-availability,marketplace-booking,marketplace-payments,marketplace-platform-infra -am test`، صفر فشل صفر خطأ في الحالتين.
...
> **دين مكتشف أثناء إغلاق B3 (2026-09-08، موروث قبل #255 — أُغلق بقرار المستخدم «نفذ الاصلاح»):** وصلة إدخال GraphQL لـ`createService` كانت منفصلة عن المخطط: حقل المخطط `price: Float!` (`schema.graphqls:29`) لا يطابق مكوّن السجل `priceCents` (`ServiceInput.java`) — ربط spring-graphql 2.0.5 (`GraphQlArgumentBinder.bindViaConstructorAndSetters`) يطابق أسماء معاملات المُنشئ بمفاتيح الإدخال **بالاسم**، فكان `priceCents=null` يصل دائمًا ويفشل `@NotNull`؛ أي أن الـ mutation رفض كل الأسعار (مدفوعة ومجانية) منذ الأصل — آخر تعديل للمخطط #226، ولم يكن أي اختبار mutation في المستودع كله. الوحدات أيضًا كانت منفصلة: المخطط يتحدث بالوحدات الكبرى (`Service.price: Float` = قروش/100 في `ServiceMapper`) بينما السجل يستقبل قروشًا.
>
> **الإغلاق (نفس جلسة قرار المستخدم):** الإدخال صار يتحدث لغة المال الموحدة للأسطح الكتابية — `priceCents: Int!` (قروش صحيحة؛ سابقة REST `priceCents` وقيد V2 `price_cents >= 0` — عائلة B3 «نفس القاعدة على كل سطح»)، والمطابقة بالاسم صارت تعمل بالبايت. جانب القراءة يبقى بوحدات العرض (`Service.price` — وحدات كبرى عبر `ServiceMapper`): عقد إدخال/إخراج مقصود — كتابة دقيقة بالقروش وقراءة عرض. **الاختبار الإلزامي المرتبط (طلب ج3 CodeRabbit) أُضيف:** `ServiceGraphQlMutationIntegrationTest` — أول اختبار mutation في المستودع (بسعر صفر إلزاميًا + موجب + سالب مرفوض + غائب مرفوض) عبر المخطط والرابط والتحقق والمعالج الحقيقيين. أُغلقت معه عيوبان مجاوران مكتشفان تجريبيًا في الجلسة الحمراء نفسها: (1) `ServiceResponse` كان بلا `category` بينما المخطط يعلن `category: String!` — طلب الحقل كان يفشل Non-Null؛ أُضيف المكوّن (تعيين MapStruct تلقائي بالاسم). (2) `GraphQlExceptionResolver` لم يكن يعالج `ConstraintViolationException` فيسقط في INTERNAL_ERROR المقنّع بينما REST تعيده VALIDATION — أُضيفت المرآاة (`GlobalExceptionHandler.handleConstraintViolation` — نفس التصنيف). التفاصيل والأدلة: جسم PR.
...
Batch C (بنية/CI):              C1→C2→C3 — **منفَّذ عبر PR #253** (C2 مُكمَّلة بجولتين: إصلاح تغليف — إزالة BOM قبل الـshebang، صلاحية 755 للسكربتين الجديدتين، وسطر نهاية الملف — ثم جولتا CodeRabbit ج3: تقويم حقيقي للتواريخ + فشل مقفول [UNREPRESENTABLE] لتغييرات المعاملات (بما فيها Add — التقرير النصي لا يُظهر required) والطلب غير الممثَّلة في قائمة (path,status) وفق دلالات ChangedParameters/ChangedParameter/ChangedContent/ChangedMediaType الرسمية؛ C3 = `V39__repair_provider_user_id_backfill.sql` + `ProviderUserIdBackfillIntegrationTest` بصيغتها النهائية بعد ج4: قطع الارتباط بالمحذوفين ناعميًا فقط، دون أي إسناد بـ`display_name` (CWE-639 — الاسم بيانات عرض متغيرة من OIDC وليس دليل ملكية؛ الصفوف بلا دليل تبقى NULL للتدبير اليدوي)؛ C4 قاعدة موثقة هنا: «لا إعادة كتابة ترحيلات مطبقة»)
```
