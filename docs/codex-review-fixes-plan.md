...
> **دين مكتشف أثناء إغلاق B3 (2026-09-08، موروث قبل #255 — يحتاج قرار مستخدم منفصل):** وصلة إدخال GraphQL لـ`createService` منفصلة عن المخطط: حقل المخطط `price: Float!` (`schema.graphqls:29`) لا يطابق مكوّن السجل `priceCents` (`ServiceInput.java`) — ربط spring-graphql 2.0.5 (`GraphQlArgumentBinder.bindViaConstructorAndSetters`) يطابق أسماء معاملات المُنشئ بمفاتيح الإدخال **بالاسم**، فيصل `priceCents=null` دائمًا ويفشل `@NotNull`؛ أي أن الـ mutation يرفض كل الأسعار (مدفوعة ومجانية) منذ الأصل — آخر تعديل للمخطط #226، ولا يوجد أي اختبار mutation في المستودع كله. الوحدات أيضًا منفصلة: المخطط يتحدث بالوحدات الكبرى (`Service.price: Float` = قروش/100 في `ServiceMapper`) بينما السجل يستقبل قروشًا. الإصلاح قرار عقد علني (اسم/وحدات حقل المخطط مقابل DTO) — خارج نطاق #255 لأنه يسبقه ولا يلمسه، موثق هنا وفق قاعدة «لا ديون مخفية»؛ **عند اعتماد إصلاحه يرتبط به وجوبًا اختبار `createService` mutation بسعر صفر** (طلب ج3 CodeRabbit — مستحيل النجاح قبل الإصلاح).
