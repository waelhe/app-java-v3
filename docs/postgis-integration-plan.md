...
| # | الدين | نقطة الإغلاق |
|---|---|---|
| D-I1 | الفهرس غير CONCURRENTLY — يسدّ الكتابة أثناء بنائه (الجدول فتيّ الآن) | ✅ **السداد هبط بالشجرة (2026-09-14، أمر «فتح D-I1..D-I5» — الدمج PR #305 ببواباته الثلاث؛ CI بالحاوية الحقيقية هو البوابة):** V51 أعاد بناء الفهرس نفسه بـ`CREATE INDEX CONCURRENTLY` (تعريف مطابق لV50 بايتًا-بايتًا) عبر **النمط الرسمي الكامل بإعداده المزدوج** (تبنّي CodeRabbit ج1 من الجذر): (أ) الملف الشقيقي `.sql.conf` يحمل `executeInTransaction=false` (وثيقة Flyway: «can be set from Script Configuration in addition to project configuration») + (ب) الإعداد المشروعي `spring.flyway.postgresql.transactional-lock: false` في application.yml (وثيقة Flyway لفضاء PostgreSQL: «This should be set to false for statements such as CREATE INDEX CONCURRENTLY» — القفل المعاملاتي الافتراضي `pg_try_advisory_xact_lock` داخل معاملة مفتوحة يسبق انتظارات اللقطات للبناء المتزامن فيعلّق الإقلاع؛ الأقفال الجلسية تُسلسل المتزامنين كما هي) — وموضع `.sql.conf` غير صالح للإعداد الثاني أصلًا (مفاتيح إعداد السكربت الأربعة فقط تُقبل — قياس بايتكود SqlScriptMetadata)؛ وبحزام ثالث: PostgreSQLParser 12.4.0 يكتشف `^(CREATE\|DROP)( UNIQUE)? INDEX CONCURRENTLY` غير-معاملاتيًا من نفسه. **قياس ما قبل التنفيذ (one-off):** فهرس الإنتاج valid/ready=true/true، مالكه `marketplace` (هوية V50 قبل فصل P3) → يصبح `flyway_migrator` مالك V51+ بتصميم ALTER DEFAULT PRIVILEGES؛ الجدول 0 صفوف. **الإغلاق الإنتاجي (وصول النشرة + إعادة قياس حالة الفهرس والملكية) يُوثَّق في §15 عند وصول النشرة — نمط دفعة #302/D-I6؛ العتبة الأصلية (>50K) لم تُطلق قط** |
