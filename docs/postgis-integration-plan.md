...
| # | الدين | نقطة الإغلاق |
|---|---|---|
| D-I1 | الفهرس غير CONCURRENTLY — يسدّ الكتابة أثناء بنائه (الجدول فتيّ الآن) | ✅ **مُغلق (2026-09-14، أمر «فتح D-I1..D-I5»):** V51 (PR #305) أعاد بناء الفهرس نفسه بـ`CREATE INDEX CONCURRENTLY` (تعريف مطابق لV50 بايتًا-بايتًا) عبر نمط `executeInTransaction=false` الرسمي (ملف `.sql.conf` شقيق — وثيقة Flyway «Execute In Transaction Setting»: «this setting can be set from Script Configuration in addition to project configuration»؛ وبحزامين: PostgreSQLParser 12.4.0 يكتشف `^(CREATE\|DROP)( UNIQUE)? INDEX CONCURRENTLY` غير-معاملاتيًا من نفسه — قياس البايتكود)؛ المسار الرسمي قبل التنفيذ: قياس one-off (فهرس الإنتاج valid/ready=true/true، مالكه `marketplace` [هوية V50 ما قبل فصل P3] → يصبح `flyway_migrator` مالك V51+ بتصميم ALTER DEFAULT PRIVILEGES؛ الجدول 0 صفوف) + النمط مثبت في CI بالحاوية الحقيقية قبل أي عتبة — العتبة الأصلية (>50K) لم تُطلق قط |
