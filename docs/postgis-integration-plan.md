...
| D-I7 | **دين مُعلن بالقياس (2026-09-14):** ملكية كائنات الترحيلات — الفهرس المعاد بناءه بV51 ملكه `marketplace` (مالك الجدول) لا `flyway_migrator` المنفّذ (`v51_installed_by` مقيسة) — آلية إسناد الملكية في مسار Flyway/PG غير مشتقة (قياسات النفي: `rolconfig` NULL للدورين، لا عضويات غير نظامية)؛ لو استمر النمط في V52+ فالمنح الافتراضية `FOR ROLE flyway_migrator` قد لا تغطي الكائنات الجديدة | **نقطة إغلاق مقيسة: أول ترحيلة V52+ تُنشئ كائنًا (جدول/عمود) تُقاس فور تطبيقها ملكية الكائن ومنح `marketplace_app` عليه — إن كانت الملكية لغير migrator والمنح ناقصة فتُوسَّع `ALTER DEFAULT PRIVILEGES` إلى الدور الذي يقيسه الواقع (فشل المنح صامت مستحيل: السطح يكسر بصوت عالٍ عند أول DML) — حتى ذلك الحين لا أثر تشغيلي (استهلاك الفهرس لا يحتاج امتيازًا — دخان 5/5)** |
