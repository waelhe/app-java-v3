...
| For a future index on a LARGE table, use `CREATE INDEX CONCURRENTLY` — which **cannot run inside Flyway's transactional wrapper**: the migration must take a non-transactional path (Flyway `executeInTransaction=false` / `flyway.postgresql.transactional.lock=false` per the official transaction-handling docs) | `CREATE INDEX` takes a SHARE lock that blocks writes for the whole build; CONCURRENTLY avoids it but refuses to run in a transaction block | [Flyway — Migration transaction handling](https://documentation.red-gate.com/flyway/flyway-concepts/migrations/migration-transaction-handling) |
