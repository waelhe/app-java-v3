...
| 9. OpenAPI documentation | Done | Full 2,139-line YAML spec (53 paths, ~50 schemas) with 6 automated validation tests; CI validation step added. |
| 10. Test Infrastructure & Quality | Done | All 17 modules pass JaCoCo 70% coverage; all 14 controllers have MockMvc tests; GlobalExceptionHandler tests exist; @SpringBootTest integration test exists. |

## Phase 9 Delivery Plan (OpenAPI)
1. ✅ Expose OpenAPI YAML endpoints in dev profile (`/v3/api-docs.yaml`, `/openapi.yaml`) and add docs landing (`/docs`).
2. ✅ Group baseline paths by module tags and document security schemes (bearer + webhook header).
3. ✅ Add smoke tests for docs endpoint/redirect behavior.
4. ✅ Expand operation request/response schemas for each controller DTO (53 paths, ~50 schemas).
5. ✅ Add automated spec consistency checks in CI (6 validation tests: paths, schemas, refs, responses, structure).

## Phase 10 Delivery Plan (Test Infrastructure & Quality)
**Official Sources:**
- https://spring.io/guides/gs/testing-web — Testing the Web Layer (MockMvc)
- https://docs.spring.io/spring-boot/reference/testing/index.html — Spring Boot Testing Reference
- https://docs.spring.io/spring-boot/reference/testing/spring-boot-applications.html#testing.spring-boot-applications.with-mock-environment — @WebMvcTest
- https://www.testcontainers.org/ — Testcontainers
- https://docs.spring.io/spring-modulith/reference/testing.html — Modulith Testing
- https://maven.apache.org/surefire/maven-surefire-plugin/ — Surefire Plugin
- https://www.jacoco.org/jacoco/ — JaCoCo Code Coverage

1. ⏭ Add `@WebMvcTest` controller tests for all 15 controllers (happy path + validation + authorization).
2. ⏭ Add unit tests for modules with 0 coverage: `marketplace-shared`, `marketplace-platform-infra`, `marketplace-catalog`.
3. ⏭ Add `@SpringBootTest` integration tests per module using Testcontainers.
4. ⏭ Add security/authorization tests (`@PreAuthorize` enforcement).
5. ⏭ Add `GlobalExceptionHandler` tests (RFC 7807 ProblemDetail responses).
6. ⏭ Add JaCoCo Maven plugin with minimum 70% coverage threshold.
7. ⏭ Verify `mvn clean verify` passes and coverage meets threshold.
