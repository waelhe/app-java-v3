...
**ملاحظة أمان جلسات للنمطين (2)/(3) (تُعرض بصدق لا تُخفى):** توكن الوصول الحي TTL=300s
(`TokenSettings`) وبلا refresh tokens للعميل العام — أي أن إعادة المصادقة ستكون متكررة عبر
...
2. سلاسل الأمن الأربعة (AS بجلسة+CSRF؛ RS بـ STATELESS) ثابتة (INV-5/6) — إضافة عميل لا تفتح سلسلة.
