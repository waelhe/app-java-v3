...
awk '
  function is_del(_) { return _ ~ /What.s Deleted/ }
  function is_chg(_) { return _ ~ /What.s Changed/ }
  /^[[:space:]]*$/ { next }
  ...
  $0 ~ /^-[[:space:]]+[A-Z]+[[:space:]]+\// {
    ...
    if (section == "chg" && NF >= 3) { method = $2; path = $3; sub(/\r/, "", path); broken_code = ""; next }
    next
  }
  section != "chg" || method == "" { next }
  $0 ~ /Return Type:/          { in_return = 1; next }
  $0 ~ /(Parameter|Request):/  { in_return = 0; next }
  in_return && /-[[:space:]]+(Changed|Deleted)[[:space:]]+[0-9]{3}/ {
    if ($2 == "Deleted") print method "\t" path "\t" $3
    else broken_code = $3
    next
  }
  broken_code != "" && /Broken compatibility/ {
