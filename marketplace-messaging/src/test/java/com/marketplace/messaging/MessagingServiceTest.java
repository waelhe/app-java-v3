...
class MessagingServiceTest {
    ...
    private final MessagingService service = new MessagingService(conversationRepository, messageRepository, bookingParticipantProvider);

    @Test
    void createConversation_savesNewWithParticipantDerivedFromBooking() {
        UUID participantA = UUID.randomUUID();
        UUID participantB = UUID.randomUUID();
        UUID bookingId = UUID.randomUUID();
        BookingInfo bookingInfo = new BookingInfo(participantB, participantA, "CONFIRMED", 5000L, "SAR", Instant.now(), Instant.now());
    ...
    void createConversation_reusesExistingForSameBooking() {
        ...
        UUID participantA = UUID.randomUUID();
        UUID participantB = UUID.randomUUID();
        BookingInfo bookingInfo = new BookingInfo(participantA, participantB, "CONFIRMED", 5000L, "SAR", Instant.now(), Instant.now());
    ...
    void createConversation_rejectsCancelledBooking() {
        UUID bookingId = UUID.randomUUID();
        UUID participantA = UUID.randomUUID();
        BookingInfo bookingInfo = new BookingInfo(participantA, UUID.randomUUID(), "CANCELLED", 5000L, "SAR", Instant.now(), Instant.now());
        when(bookingParticipantProvider.getBookingInfo(bookingId)).thenReturn(bookingInfo);
    ...
    void createConversation_rejectsWhenNotParticipant() {
        UUID bookingId = UUID.randomUUID();
        UUID participantA = UUID.randomUUID();
        BookingInfo bookingInfo = new BookingInfo(UUID.randomUUID(), UUID.randomUUID(), "CONFIRMED", 5000L, "SAR", Instant.now(), Instant.now());
        when(bookingParticipantProvider.getBookingInfo(bookingId)).thenReturn(bookingInfo);

        assertThrows(AccessDeniedException.class, () -> service.createConversation(participantA, bookingId));
    }
